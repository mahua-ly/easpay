...
public class HomeController {
    ...
    @Autowired
    public HomeController(CommonService commonService, LoginService loginService, UserAuthInfoService userAuthInfoService, WeChatConfig weChatConfig) {
    ...
    public ResponseResult weChatAuthCallBack(HttpServletRequest request){
        ...
        String url = weChatConfig.getGetAccessTokenUrl().replace("APPID", weChatConfig.getAppId()).replace("SECRET", weChatConfig.getAppSecret()).replace("CODE", code);
        ...
        if (StringUtils.isNotBlank(tokenDTO.getAccess_token()) && StringUtils.isNotBlank(tokenDTO.getOpenid())) {
            //4. 拉取用户微信信息URL拼接
            String pullUserInfoUrl = weChatConfig.getSnsUserinfoUrl().replace("ACCESS_TOKEN", tokenDTO.getAccess_token()).replace("OPENID", tokenDTO.getOpenid());
